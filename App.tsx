...
const processScheduleForSchoolView = (
    plans: SemesterCourse[],
    state: AppState
): any[] => {
    const { courses, teachers, rooms, studentGroups, schedule } = state;
    const data: any[] = [];
    if (!plans) return [];
...
const SchoolScheduleTable: React.FC<{ data: any[] }> = ({ data }) => {
    if (data.length === 0) return <div className="text-center text-gray-500 dark:text-gray-400">No hay datos para mostrar.</div>;
    return (
        <div className="overflow-x-auto">
...
const SchoolScheduleView: React.FC<{ state: AppState }> = ({ state }) => {
    const { semesterPlan } = state;

    const processedData = useMemo(() => {
        if (!semesterPlan) return [];
        const activePlans = semesterPlan.filter(p => p.isActive && !p.isReprogrammed);
        return processScheduleForSchoolView(activePlans, state);
    }, [semesterPlan, state]);

    const reprogrammedData = useMemo(() => {
        if (!semesterPlan) return [];
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg space-y-8">
            ...
            {reprogrammedData.length > 0 && (
                 <div>
                    <div className="text-center mb-4">
                        <h3 className="text-xl font-bold text-gray-800 dark:text-gray-100">Asignaturas Reprogramadas</h3>
                    </div>
                    <SchoolScheduleTable data={reprogrammedData} />
                </div>
            )}
        </div>
    );
